        ...
        if ctx.command != None:
            ...
            if ctx.command.name not in server['mod']['off_commands']:
                if functions.cooldown_check(message.author, message.guild, ctx.command.name, 'check') == False:
                    ...
                    emb = discord.Embed(title = '⏲️ | Режим ожидания', description = f"Команда `{ctx.command}` может быть активированна повторно <t:{int(time.time() + tt)}:R>", color = server['embed_color'])
